...
<button id="lv-chat-fab" onclick="lvToggleChatbot()" title="Chat with LifeVault AI">
    <span id="lv-chat-fab-icon" style="background:transparent;"><img src="{{ asset('logo.svg') }}" alt="AI" style="width:80px;height:80px;border-radius:50%;object-fit:contain;display:block;min-width:80px;min-height:80px;"></span>
</button>
...
(function(){
    ...
    function getAuthStatus(){
        ...
        if(fbUser) return {isLoggedIn:true,name:fbUser.displayName||'Member'};
        if(window.currentUser) return {isLoggedIn:true,name:window.currentUser.displayName||'Member'};
        return {isLoggedIn:false,name:''};
    }
    function buildSystemPrompt(){
        var auth=getAuthStatus();
        var ctx=auth.isLoggedIn
            ? `USER CONTEXT: Signed in as "${auth.name}". DO NOT tell them to sign in or register under any circumstances.`
            : `USER CONTEXT: Guest visitor.`;
        return `You are LifeVault AI — the intelligent personal growth assistant for LifeVault, a private AI-powered journaling and self-discovery web app. Tools: 📓 Journal, 🔮 Shadow Self Analyzer, 📖 Life Story Generator, ✨ Holistic Career Advisor, 📄 Resume Analyzer, ✅ Tasks, 🎯 Goals, 🔖 Saved Items. Be warm, insightful, encouraging, and helpful. ${ctx}`;
    }
    function renderMarkdown(t){return typeof marked!=='undefined'?marked.parse(t):t.replace(/\n/g,'<br>');}
    function getTime(){return new Date().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'});}
    function appendAIMsg(text,isTyping){var wrap=document.createElement('div');wrap.style.cssText='display:flex;flex-direction:column;gap:3px';if(isTyping)wrap.id='lv-typing-wrap';var row=document.createElement('div');row.className='lv-chat-msg lv-chat-ai';var ico=document.createElement('div');ico.className='lv-chat-ico';ico.innerHTML='<img src="/logo.svg" alt="AI" style="width:64px;height:64px;border-radius:50%;object-fit:contain;display:block;min-width:64px;min-height:64px;">';var bubble=document.createElement('div');bubble.className='lv-chat-bubble';bubble.innerHTML=isTyping?'<span class="lv-chat-typing"><span></span><span></span><span></span></span>':'<p>'+renderMarkdown(text)+'</p>';row.appendChild(ico);row.appendChild(bubble);wrap.appendChild(row);if(!isTyping){var m=document.createElement('div');m.className='lv-chat-meta';m.style.paddingLeft='40px';m.textContent=getTime();wrap.appendChild(m);}messages.appendChild(wrap);messages.scrollTop=messages.scrollHeight;return bubble;}
    ...
    function seedWelcome(){if(!messages||messages.children.length)return;var a=getAuthStatus();appendAIMsg("Hi"+(a.name?" "+a.name.split(' ')[0]:"")+"! I'm **LifeVault AI** ✨\n\nAsk me anything — journaling guidance, shadow work, career advice, or how any feature works.\n\nWhat would you like to explore today?");}
    window.lvToggleChatbot=function(){_chatOpen=!_chatOpen;overlay.classList.toggle('lv-chat-visible',_chatOpen);fab.classList.toggle('lv-chat-open',_chatOpen);document.getElementById('lv-chat-fab-icon').innerHTML=_chatOpen?'✕':'<img src="/logo.svg" alt="AI" style="width:80px;height:80px;border-radius:50%;object-fit:contain;display:block;min-width:80px;min-height:80px;">';if(_chatOpen){seedWelcome();setTimeout(function(){if(input)input.focus();},350);}};
    window.lvSendChat=function(){if(!input||_isTyping)return;var text=input.value.trim();if(!text)return;input.value='';sendBtn.disabled=true;_isTyping=true;appendUserMsg(text);if(_chatHistory.length>18)_chatHistory=_chatHistory.slice(-18);_chatHistory.push({role:'user',content:text});var bubbleEl=appendAIMsg('',true);var typingWrap=document.getElementById('lv-typing-wrap');var accumulated='',started=false;
        fetch('https://openrouter.ai/api/v1/chat/completions',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer {{ env("OPENROUTER_API_KEY") }}','HTTP-Referer':'{{ config("app.url") }}','X-Title':'LifeVault'},body:JSON.stringify({model:'nex-agi/nex-n2.5-pro:free',max_tokens:800,stream:true,messages:[{role:'system',content:buildSystemPrompt()}].concat(_chatHistory)})})
