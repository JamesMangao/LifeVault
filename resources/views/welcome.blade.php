...
/* ── Chatbot FAB ─────────────────────────────────────────── */
#lv-chat-fab{position:fixed;bottom:32px;right:32px;z-index:9999;width:58px;height:58px;border-radius:50%;border:none;cursor:pointer;background:linear-gradient(135deg,#7c3aed,#4f8ef7);box-shadow:0 8px 32px rgba(124,58,237,.5);display:flex;align-items:center;justify-content:center;font-size:1.5rem;transition:transform .25s cubic-bezier(.34,1.56,.64,1),box-shadow .25s;animation:lv-fadeup .7s .8s ease both}
...
(function(){
    ...
    function appendAIMsg(text,isTyping){var wrap=document.createElement('div');wrap.style.cssText='display:flex;flex-direction:column;gap:3px';if(isTyping)wrap.id='lv-typing-wrap';var row=document.createElement('div');row.className='lv-chat-msg lv-chat-ai';var ico=document.createElement('div');ico.className='lv-chat-ico';ico.innerHTML='<img src="/logo.svg" alt="AI" style="width:32px;height:32px;border-radius:50%;object-fit:cover;display:block;">';var bubble=document.createElement('div');bubble.className='lv-chat-bubble';bubble.innerHTML=isTyping?'<span class="lv-chat-typing"><span></span><span></span><span></span></span>':'<p>'+renderMarkdown(text)+'</p>';row.appendChild(ico);row.appendChild(bubble);wrap.appendChild(row);if(!isTyping){var m=document.createElement('div');m.className='lv-chat-meta';m.style.paddingLeft='40px';m.textContent=getTime();wrap.appendChild(m);}messages.appendChild(wrap);messages.scrollTop=messages.scrollHeight;return bubble;}
    ...
    window.lvToggleChatbot=function(){_chatOpen=!_chatOpen;overlay.classList.toggle('lv-chat-visible',_chatOpen);fab.classList.toggle('lv-chat-open',_chatOpen);document.getElementById('lv-chat-fab-icon').innerHTML=_chatOpen?'✕':'<img src="/logo.svg" alt="AI" style="width:36px;height:36px;border-radius:50%;object-fit:cover;display:block;">';if(_chatOpen){seedWelcome();setTimeout(function(){if(input)input.focus();},350);}};
