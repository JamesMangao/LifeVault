...
<button id="lv-chat-fab" onclick="lvToggleChatbot()" title="Chat with LifeVault AI">
    <span id="lv-chat-fab-icon" style="background:transparent;"><img src="{{ asset('logo.svg') }}" alt="AI" style="width:36px;height:36px;border-radius:50%;object-fit:contain;display:block;min-width:36px;min-height:36px;"></span>
</button>
...
(function(){
    ...
    window.lvToggleChatbot=function(){_chatOpen=!_chatOpen;overlay.classList.toggle('lv-chat-visible',_chatOpen);fab.classList.toggle('lv-chat-open',_chatOpen);document.getElementById('lv-chat-fab-icon').innerHTML=_chatOpen?'✕':'<img src="/logo.svg" alt="AI" style="width:36px;height:36px;border-radius:50%;object-fit:contain;display:block;min-width:36px;min-height:36px;">';if(_chatOpen){seedWelcome();setTimeout(function(){if(input)input.focus();},350);}};
